...
#define NUMBER_OF_FRAMES 48
#define NUMBER_OF_BYTES 8

const uint8_t animation[NUMBER_OF_ANIMATIONS][NUMBER_OF_FRAMES][NUMBER_OF_BYTES] PROGMEM = {
};
...
void updateLightShow(MachineState& machineState) {
    if (!Time::get1000msIT()) return;
    for (uint8_t i = 0; i < NUMBER_OF_ANIMATIONS; i++) {
        for (uint8_t j = 0; j < NUMBER_OF_FRAMES; j++) {
            for (uint8_t k = 0; k < NUMBER_OF_FRAMES; k++) {
                LampsHelper::setLampState(pgm_read_byte(&animation[i][j][k]), true, 0, 0);
            }
        }
    }
}
